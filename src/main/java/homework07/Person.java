...
public class Person {
    ...
    /**
     * метод добавляет продукты покупателю в корзину и вычисляет остаток денег
     * @param product
     */
    ...
    public String toString() {
        ...
        StringBuilder result = new StringBuilder(name + " купил(а):\n");
        double totalSpent = 0;
        double totalSaved = 0;

        for (Product product : bag) {
            result.append("  - ").append(product.toString()).append("\n");
            totalSpent += product.getPrice();

            // Считаем сэкономленные деньги для скидочных продуктов
            if (product instanceof DiscountProduct) {
                ...
                if (discountProduct.isDiscountActive()) {
                    totalSaved += discountProduct.getDiscountAmount();
                }
            }
        }

        result.append(String.format("Итого потрачено: %.2f руб.\n", totalSpent));
        if (totalSaved > 0) {
            result.append(String.format("Сэкономлено благодаря скидкам: %.2f руб.\n", totalSaved));
        }
        result.append(String.format("Осталось денег: %.2f руб.", money));
